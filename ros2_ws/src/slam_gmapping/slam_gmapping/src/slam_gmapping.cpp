...
void SlamGmapping::init() {
    ...
    // Defaults match ROS 1 slam_gmapping; all overridable via ROS parameters.
    ...
    map_update_interval_ = tf2::durationFromSec(
        this->declare_parameter("map_update_interval", 0.5));
    // <= 0 means: derive from the first scan's range_max (see initMapper).
...
void SlamGmapping::startLiveSlam() {
    ...
    particles_pub_ = this->create_publisher<geometry_msgs::msg::PoseArray>("particles", rclcpp::SystemDefaultsQoS());
...
bool SlamGmapping::initMapper(const sensor_msgs::msg::LaserScan::ConstSharedPtr scan)
{
    ...
    if (maxRange_ <= 0.0) maxRange_ = scan->range_max - 0.01;
...
void SlamGmapping::laserCallback(sensor_msgs::msg::LaserScan::ConstSharedPtr scan) {
    ...
    bool updated = addScan(scan, odom_pose);
    double update_ms = std::chrono::duration<double, std::milli>(
        std::chrono::steady_clock::now() - t_update_start).count();

    if(updated)
    {
        // --- comparison logging: runtime + normalized weight spread ---
        // Weights normalized with the same temper GridSlamProcessor::normalize()
        // uses (gain = 1/(ogain * N)) so [min,max] matches the fastslam node.
        const auto& particles = gsp_->getParticles();
        int n = static_cast<int>(particles.size());
        double lmax = -std::numeric_limits<double>::max();
        double w_raw_min = std::numeric_limits<double>::max();
        for (const auto& p : particles) {
            if (p.weight > lmax) lmax = p.weight;
            if (p.weight < w_raw_min) w_raw_min = p.weight;
        }
        double gain = 1.0 / (ogain_ * n);
        double sum = 0.0;
        for (const auto& p : particles) sum += std::exp((p.weight - lmax) * gain);
        double w_max_norm = 1.0 / sum;                                  // best particle (weight == lmax)
        double w_min_norm = std::exp((w_raw_min - lmax) * gain) / sum;
        // Print on std::cout (same stream as GMapping's library logs) so
        // ros2 launch doesn't interleave it out of order with them.
        std::cout << "[gmapping] " << update_ms << "ms | weights=["
                  << w_min_norm << ", " << w_max_norm << "] | particles=" << n
                  << std::endl;

        // --- publish the particle cloud (map frame), like the fastslam node ---
        geometry_msgs::msg::PoseArray cloud;
        cloud.header.stamp = scan->header.stamp;
        cloud.header.frame_id = map_frame_;
        cloud.poses.reserve(particles.size());
        for (const auto& p : particles) {
            ...
            pose.position.z = 0.0;
            tf2::Quaternion pq;
            pq.setRPY(0, 0, p.pose.theta);
            pose.orientation.x = pq.x();
            pose.orientation.y = pq.y();
            pose.orientation.z = pq.z();
            pose.orientation.w = pq.w();
